...
while(1)
    ...
    % data1 receives PT1
    data1(i) = str2double(str{2})*1.0533*10^(-4);
    % data2 receives PT2
    data2(i) = str2double(str{3})*1.0323*10^(-4);
    % data3 receives PT3
    data3(i) = str2double(str{4})*2.3013*10^(-5);
    % data4 receives PT4
    data4(i) = str2double(str{5})*2.3013*10^(-5);
    % data5 receives LC1
    data5(i) = str2double(str{6})*2.3013*10^(-5);
    % data6 receives LC2
    data6(i) = str2double(str(7))*2.3013*10^(-5);
    % data7 receives LC3
    data7(i) = str2double(str(8))*2.3013*10^(-5);
    ...
    if (now() - timeControl) * 24 * 60 * 60 >= plotTime % plot every x seconds
        if timeInterval(end)-timeInterval(1) <= observationInterval
            ...
            axes(ax7);
            plot(timeInterval(end-observationInterval/pauseTime:end),data7(end-observationInterval/pauseTime:end));
            % set the x limits so that only the last 5 seconds of data is
            % plotted
            title('Load Cell 3')
            ...
            axes(ax8);
            plot(timeInterval(end-observationInterval/pauseTime:end),data8(end-observationInterval/pauseTime:end));
            % set the x limits so that only the last 5 seconds of data is
            % plotted
            title('Flowmeter')
